...
const Stats = () => {
  return (
    <section className="relative -mb-2">
      ...
      <div className="absolute grid grid-cols-stats gap-x-[20vw] 3xl:gap-x-[390px]  gap-y-[6vw]  top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        {elements.map((el, i) => (
          <p key={i} className="text-center text-white text-2xl">
            <span className="block text-6xl font-bold">{el.title}</span>
            {el.text}
          </p>
        ))}
      </div>
